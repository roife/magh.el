...
(ert-deftest magh-commit-review-renderer-shows-threads-drafts-and-stale-state ()
  (let* ((context (magh-context-from-repository "o/r"))
    ...
    (with-temp-buffer
      ...
      (let* ((reviews
              (seq-find (lambda (section)
                          (eq (oref section type) 'reviews))
                        (oref magit-root-section children)))
             (review-sections (oref reviews children))
             (changed-files
              (seq-find (lambda (section)
                          (eq (oref section type) 'changed-files))
                        (oref magit-root-section children)))
        ...
        (should (= (length review-sections) 2))
        (let ((root-review (car review-sections))
              (reply-review (cadr review-sections)))
          (dolist (review review-sections)
            (should (oref review hidden))
            (should-not (magh-commit--section-resource review))
            (magit-section-show review)
            (should-not (oref review hidden))
            (should (equal (mapcar (lambda (section) (oref section type))
                                   (oref review children))
                           '(review-link))))
          (setq text (buffer-string))
          (let ((root-link (car (oref root-review children)))
                (reply-link (car (oref reply-review children))))
            (dolist (link (list root-link reply-link))
              (should (eq (plist-get (magh-commit--section-resource link) :kind)
                          'review-summary)))
            (goto-char (oref root-link start))
            (magh-resource-open (magh-ui-resource-at-point))
            (should (eq (oref (magit-current-section) type) 'inline-comment))
            (should (= (plist-get (magh-ui-resource-at-point) :root-id) 10))
            (goto-char (oref reply-link start))
            (magh-resource-open (magh-ui-resource-at-point)))
          (should (eq (oref (magit-current-section) type) 'comment))
          (should (= (magh-section-value-key
                      (oref (magit-current-section) value))
                     11)))))
    (should (string-match-p "PR #7  Review me" text))
    (should (string-match-p "APPROVED by alice" text))
    (should (string-match-p "COMMENTED by alice" text))
    (should (string-match-p "(1 linked comment)" text))
    (should (string-match-p "Summary" text))
    (should (string-match-p "src/a.el:1 bob #10" text))
    (should (string-match-p "src/a.el:1 alice #11" text))
    (should (string-match-p
